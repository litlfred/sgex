 ...
 * 
 * Consistent header component for all pages with user controls, bookmarks, and navigation
 * 
 * @module PageHeader
 */
...
/**
 * User information
 * @example { login: "user123", name: "John Doe", avatar_url: "https://...", html_url: "https://github.com/user123" }
 */
...
/**
 * Grouped bookmarks by page
 * @example { pageName: "Business Processes", bookmarks: [...] }
 */
export interface BookmarkGroup {
  /** Page name */
  pageName: string;
  /** List of bookmarks for this page */
  bookmarks: Bookmark[];
}

/**
 * Consistent header component for all pages
 * Provides user controls, bookmarks, navigation, and preview badge
 * 
 * Features:
 * - SGEX Workbench branding with home navigation
...
const PageHeader: React.FC = () => {
  ...
  const [samlModalInfo, setSamlModalInfo] = useState<SAMLModalInfo | null>(null);
  ...
  // Use refs to ensure callback always has current state setters
  ...
  // Register SAML modal callback IMMEDIATELY (in useState initializer)
  useState(() => {
    samlAuthService.registerModalCallback((samlInfo: SAMLModalInfo) => {
      ...
      // Open modal with SAML info (for hosted mode)
      setModalInfoRef.current({
        ...samlInfo,
        isSPAMode: !repositoryConfig.isSAMLSupported()
      });
      setModalOpenRef.current(true);
    });
    return true; // Return value for useState
  });
  ...
  const handleWHOSAMLAuth = (): void => {
    ...
    if (!isSAMLSupported) {
      // In SPA mode, show informational modal
      setSamlModalInfo({
  ...
  const handleBookmarkNavigation = (bookmark: Bookmark): void => {
