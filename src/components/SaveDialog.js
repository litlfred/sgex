...
const SaveDialog = ({ 
  ...
  const checkPATStatus = useCallback(async () => {
    ...
    try {
      ...
      // Check if we have a suitable PAT for this repository
      const suitablePAT = await patManagementService.findBestPATForRepository(owner, repo);
      
      if (suitablePAT) {
        const hasWrite = await patManagementService.checkRepositoryWriteAccess(owner, repo);
        setPATStatus({
          hasPAT: true,
      ...
      console.warn('Error checking PAT status:', error);
      setPATStatus({
        hasPAT: false,
        hasWriteAccess: false,
        patInfo: null
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Check PAT status for this repository
      checkPATStatus();
    }
  }, [isOpen, stagingGround, repository, checkPATStatus]);
  ...
  const effectiveWriteAccess = hasWriteAccess && patStatus?.hasWriteAccess !== false;
  ...
  const handleSave = async () => {
    ...
    if (!effectiveWriteAccess) {
      setError('You need write permissions to save changes. Please ensure you have a PAT with write access for this repository.');
      return;
    }
    ...
    try {
      ...
      // Create commit with multiple files using PAT management service
  ...
  const getErrorMessage = (error) => {
    if (error.status === 409) {
      ...
      return 'Permission denied. Check that your PAT has write access to this repository. You may need to add a token with write permissions.';
    } else if (error.status === 404) {
      return 'Repository or branch not found. Please verify the repository exists and you have access.';
    } else if (error.status === 401) {
      return 'Authentication failed. Your PAT may have expired or been revoked. Please check your token settings.';
      ...
      <div className="save-dialog">
        ...
        <div className="dialog-content">
          ...
          <div className="save-summary">
            ...
            {stagingGround?.files?.length === 0 && (
              <div className="no-files">
                <span className="no-files-icon">📂</span>
                <span className="no-files-message">No files to commit</span>
              </div>
            )}
          </div>
          ...
          {/* PAT Status */}
          {patStatus && (
            <div className="pat-status-section">
              <h4>🔐 Access Status</h4>
              <div className={`pat-status ${patStatus.hasWriteAccess ? 'write-access' : 'no-write-access'}`}>
                {patStatus.hasPAT ? (
                  <>
                    <span className="status-icon">
                      {patStatus.hasWriteAccess ? '✏️' : '👁️'}
                    </span>
                    <span className="status-text">
                      {patStatus.hasWriteAccess ? 'Write access available' : 'Read-only access'}
                    </span>
                    {patStatus.patInfo && (
                      <span className="pat-info">
                        using {patStatus.patInfo.username || 'PAT'}
                      </span>
                    )}
                  </>
                ) : (
                  <>
                    ...
                    <span className="status-text">No PAT available for this repository</span>
                  </>
                )}
              </div>
              {!patStatus.hasWriteAccess && (
                <div className="pat-warning">
                  ⚠️ You need a PAT with write permissions to save changes to this repository.
                </div>
              )}
            </div>
          )}
          ...
          <div className="repo-info">
            ...
            <div className="info-item">
              ...
              <span className={`info-value ${effectiveWriteAccess ? 'write' : 'read'}`}>
                {effectiveWriteAccess ? '✏️ Write' : '👁️ Read-only'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="dialog-actions">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={!canSave || !commitMessage.trim() || isSaving || !effectiveWriteAccess}
          >
            {isSaving ? (
              <>
