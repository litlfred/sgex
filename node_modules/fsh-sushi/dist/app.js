...
async function runUpdateDependencies(projectPath) {
    ...
    const registryClient = new fhir_package_loader_1.DefaultRegistryClient({ log: utils_1.logMessage });
    await (0, utils_1.updateExternalDependencies)(config, registryClient);
}
async function runBuild(input, program, helpText) {
    ...
    // Load dependencies
    const defs = await (0, fhirdefs_1.createFHIRDefinitions)(false);
    ...
    await (0, ig_1.loadPredefinedResources)(defs, path_1.default.resolve(input, '..'), path_1.default.resolve(originalInput), config.parameters);
