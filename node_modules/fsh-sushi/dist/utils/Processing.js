...
async function updateExternalDependencies(config, registryClient) {
    var _a;
    ...
    const promises = config.dependencies.map(async (dep) => {
        if (/@npm/.test(dep.packageId)) {
            FSHLogger_1.logger.info(`Skipping dependency version check for NPM aliased package: ${dep.packageId}`);
        }
...
async function loadExternalDependencies(defs, config) {
    ...
    ((_a = config.dependencies) !== null && _a !== void 0 ? _a : []).forEach(dep => {
        var _a;
        const npmAliasMatcher = (_a = dep.packageId) === null || _a === void 0 ? void 0 : _a.match(/^([^@]+)@npm:(.+)$/);
        if (npmAliasMatcher) {
            const [alias, packageId] = npmAliasMatcher.slice(1);
            if (!/^[A-Za-z0-9\-\.]+$/.test(alias)) {
                FSHLogger_1.logger.warn(`NPM aliases should contain only the following characters: upper- or lower-case ASCII letters ('A'..'Z', and 'a'..'z'), numerals ('0'..'9'), '-' and '.'. Found '${alias}'.`);
            }
    ...
    packageIdMap.forEach(dependsOns => {
        dependencies.push(...dependsOns.sort((a, b) => semver_1.default.compareLoose(a.version, b.version)));
    });
...
async function loadAutomaticDependencies(fhirVersion, configuredDependencies, defs) {
    ...
    if (fhirVersionName === 'R4' || fhirVersionName === 'R4B') {
        ...
        const R5forR4Map = new Map();
        fhirdefs_1.R5_DEFINITIONS_NEEDED_IN_R4.forEach(def => R5forR4Map.set(def.id, def));
        const virtualR5forR4Package = new fhir_package_loader_1.InMemoryVirtualPackage({ name: 'sushi-r5forR4', version: '1.0.0' }, R5forR4Map, {
            log: (level, message) => {
                (0, FSHLogger_1.logMessage)(level, message);
            }
        });
    ...
    for (const dep of exports.AUTOMATIC_DEPENDENCIES) {
        ...
        if (!alreadyConfigured) {
            ...
            try {
                ...
                defs.setFHIRPackageLoaderLogInterceptor((level) => {
                    return level !== 'error';
                });
...
function writeFSHIndex(outDir, outPackage, inputDir, skippedResources = []) {
    ...
    fs_extra_1.default.ensureDirSync(path_1.default.join(outDir, 'fsh-generated', 'data'));
    fs_extra_1.default.outputJsonSync(path_1.default.join(outDir, 'fsh-generated', 'data', 'fsh-index.json'), jsonIndex, {
        spaces: 2
    });
}
