...
class InstanceExporter {
    ...
    setAssignedValues(fshInstanceDef, instanceDef, instanceOfStructureDefinition) {
            ...
            if (r instanceof rules_1.AssignmentRule && r.isInstance) {
                ...
                if (instance != null) {
                    if (instance._instanceMeta.usage === 'Example' &&
                        ...
                        utils_1.logger.warn(`Contained instance "${r.value}" is an example and probably should not be included in a conformance resource.`, r.sourceInfo);
                    }
                    r.value = instance;
                    return true;
                }
        ...
        rules.forEach(rule => {
            ...
            const doRuleValidation = (value) => {
                ...
                if (value instanceof fshtypes_1.FshCanonical) {
                    const entityName = value.entityName;
                    const matchingContainedReferenceId = (0, common_1.getMatchingContainedReferenceId)(entityName, containedRules);
            ...
            try {
                ...
                if (rule instanceof rules_1.AssignmentRule &&
                    ...
                    if (instanceToAssign == null) {
                        ...
                        try {
                            if (instanceToAssign._instanceMeta.usage === 'Example' &&
                                instanceDef._instanceMeta.usage === 'Definition') {
                                utils_1.logger.warn(`Contained instance "${rule.rawValue}" is an example and probably should not be included in a conformance resource.`, rule.sourceInfo);
                            }
        ...
        if (instanceOfStructureDefinition.url ===
            ...
            (0, common_1.setImpliedPropertiesOnInstance)(instanceDef, instanceOfStructureDefinition, secretPaths, inlineResourcePaths.map(p => p.path), this.fisher, knownSlices, manualSliceOrdering);
        }
        else {
    ...
    validateRequiredChildElements(instance, element, fshDefinition, parentPrimitive) {
        ...
        children.forEach(c => {
            ...
            if (instanceChild == null && childPathEnd.endsWith('[x]')) {
                ...
                const choiceSlices = possibleChoiceSlices.filter(c => c.path === child.path && (c.sliceName || c.type.length === 1));
                for (const choiceSlice of choiceSlices) {
                    ...
                    const sliceName = (_a = choiceSlice.sliceName) !== null && _a !== void 0 ? _a : childPathEnd.replace('[x]', (0, lodash_1.upperFirst)(choiceSlice.type[0].code));
                    // as above, we use the _ prefixed element if it exists
                    instanceChild = (_b = instance[`_${sliceName}`]) !== null && _b !== void 0 ? _b : instance[sliceName];
                ...
                if (instanceChild == null) {
                    const namelessChoiceSlices = (_d = (_c = element
                        .findConnectedSliceElement()) === null || _c === void 0 ? void 0 : _c.children(true).filter(c => c.path === child.path && c.sliceName)) !== null && _d !== void 0 ? _d : [];
                ...
                if (instanceChild == null) {
                    for (const type of child.type) {
                        ...
                        instanceChild = (_e = instance[`_${name}`]) !== null && _e !== void 0 ? _e : instance[name];
                        if (instanceChild != null) {
                            break;
                        }
                    }
            ...
            if ((child.min > 0 &&
                ...
                !(child.id.endsWith('.value') &&
                    (typeof parentPrimitive === 'object'
                        ? parentPrimitive === null || parentPrimitive === void 0 ? void 0 : parentPrimitive.assignedValue
    ...
    fishForMetadatas(item) {
        // If it's in the tank, it can get the metadata from there (no need to export like in fishForFHIR)
        return this.fisher.fishForMetadatas(item, utils_1.Type.Instance);
    }
    applyInsertRules() {
    ...
    exportInstance(fshDefinition) {
        ...
        let instanceOfStructureDefinition;
        if (this.sdCache.has(json.url)) {
            instanceOfStructureDefinition = this.sdCache.get(json.url);
        }
        else {
            instanceOfStructureDefinition = fhirtypes_1.StructureDefinition.fromJSON(json);
        ...
        if (this.shouldSetMetaProfile(instanceDef, instanceOfStructureDefinition)) {
            ...
            if (!((_b = (_a = instanceDef.meta) === null || _a === void 0 ? void 0 : _a.profile) === null || _b === void 0 ? void 0 : _b.some((profile) => {
                const profileUrl = typeof profile === 'object' ? profile === null || profile === void 0 ? void 0 : profile.assignedValue : profile;
                return (profileUrl === instanceOfStructureDefinition.url ||
                    (profileUrl === null || profileUrl === void 0 ? void 0 : profileUrl.startsWith(`${instanceOfStructureDefinition.url}|`)));
            }))) {
                ...
                if (instanceDef.meta == null) {
                    ...
                    if (((_c = instanceDef.meta._profile) === null || _c === void 0 ? void 0 : _c.length) > 0) {
                        if ((0, lodash_1.isEmpty)(instanceDef.meta.profile[0])) {
        ...
        (0, common_1.checkForMultipleChoice)(fshDefinition, instanceDef, instanceOfStructureDefinition);
        ...
        if ((_d = instanceDef.meta) === null || _d === void 0 ? void 0 : _d.profile) {
            if (instanceDef.meta._profile) {
                const complexProfiles = (0, lodash_1.zip)(instanceDef.meta.profile, instanceDef.meta._profile);
                const uniqueProfiles = (0, lodash_1.uniqWith)(complexProfiles, (a, b) => {
                    return a[0] != null && a[0] === b[0];
                });
                const removedProfiles = (0, lodash_1.differenceWith)(complexProfiles, uniqueProfiles, lodash_1.isEqual);
                removedProfiles.forEach(rp => {
                    const homeIndex = uniqueProfiles.findIndex(up => up[0] === rp[0]);
                    (0, lodash_1.mergeWith)(uniqueProfiles[homeIndex][1], rp[1], (a, b) => {
                ...
                [instanceDef.meta.profile, instanceDef.meta._profile] = (0, lodash_1.unzip)(uniqueProfiles);
            }
            else {
        ...
        if (instanceDef._instanceMeta.usage !== 'Inline' &&
            ...
            utils_1.logger.error(`Multiple instances of type ${instanceDef.resourceType} with id ${(_e = instanceDef.id) !== null && _e !== void 0 ? _e : instanceDef._instanceMeta.name}. Each non-inline instance of a given type must have a unique id.`, fshDefinition.sourceInfo);
        }
        return instanceDef;
    }
    /**
    ...
    export() {
        ...
        for (const instance of instances) {
            try {
                ...
                utils_1.logger.error(e.message, instance.sourceInfo);
                if (e.stack) {
