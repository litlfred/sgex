...
export class DAKFAQMCPServer {
    ...
    async executeQuestion(args) {
        ...
        try {
            ...
            const result = await this.faqEngine.executeSingle(request, parsed.context || {});
        ...
        const content = [
            {
                ...
                text: `# Parameter Schema for ${question.title}\n\n` +
                    ...
                    `## Parameter Details\n\n` +
                    question.parameters.map(p => `- **${p.name}** (${p.type}): ${p.description}\n` +
